...
class Transaction:
  ...
  def __init__(
    ...
    self.ingest_time: datetime = ingest_time or datetime.now(timezone.utc)

  @staticmethod
  def _validate(
      amount_minor: int, currency: str, event_time: datetime
  ) -> None:
    if amount_minor <= 0:
      raise ValueError(
          f"Transaction amount must be positive, got: {amount_minor}"
      )
    if currency not in VALID_CURRENCIES:
      raise ValueError(
          f"Invalid currency: '{currency}'. Must be one of"
          f" {set(VALID_CURRENCIES)}"
      )
  ...
  def from_bytes(cls, raw: bytes) -> "Transaction":
    ...
    try:
      ...
      ingest_time = (
          datetime.fromisoformat(data["ingest_time"])
  ...
  def kafka_key(self) -> bytes:
    if not self.account_id:
      raise ValueError(
          "Cannot generate Kafka key: account_id is empty or None"
      )
